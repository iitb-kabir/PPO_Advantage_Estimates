...
from analysis.critic_hessian import validate_critic_bound
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--skip-matched-gae",
        action="store_true",
        help="Skip the deployment (matched-policy) GAE-bias table.",
    )
    parser.add_argument("--skip-critic-hessian", action="store_true")
...
def main() -> None:
    ...
    if not args.skip_evaluation:
        evaluate_all_models(sigmas=args.sigmas, seeds=args.seeds, episodes=args.eval_episodes)
    if not args.skip_gae:
        # Theory test: fixed nominal (sigma=0) policy under increasing turbulence.
        compute_bias_table(
            sigmas=args.sigmas,
            seeds=args.seeds,
        ...
        if not args.skip_matched_gae:
            # Deployment test: the policy trained at each sigma, evaluated at that sigma.
            compute_bias_table(
                sigmas=args.sigmas,
                seeds=args.seeds,
                n_episodes=args.gae_episodes,
    ...
    fit_scaling_models()
    if not args.skip_critic_hessian:
        validate_critic_bound(seeds=args.seeds)
